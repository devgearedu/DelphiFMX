...
//     FREEWARE - Aucune diffusion  commerciale bas? sur ce code source
//     n'est autoris? sans autorisation pr?lable de l'auteur.
...
type
  ...
  private
    { D?larations priv?s }
    LastDirection: ShortInt;  // Sens de la derni?e rotation  90 Aig -90 Inv
...
/// *****************************************************
/// Rechercher les cubes Coins et Arretes situ? sur le
/// m?e plan que le cube allant pivoter et en faire ses
/// enfants.
...
var
  I: Integer;
begin
  // r?uperation du sens de la rotation
...
/// *************************************************
/// En fin d'animation ROTATION le cube ayant pivot?
...
begin
  TheStick.ResetRotationAngle; // Remise ?zero des angles de rotation  IMPORTANT
...
begin
  IF MvtOn then
    Exit;
  if ssLeft in Shift then // si la souris bouge avec le clic droit enfonc?
  begin
    Direction := DiNone;
    IF (ClickX - X) > 3 then
      Direction := DiLeft;
    ...
    if Direction <> DiNone then // une direction a ??trouv?
    begin
      ClickX := X;
      ClickY := Y;
...
begin
  if not MvtOn then
    Exit; // si pas de mouvement d'arm?
...
{$ENDREGION}
{$REGION 'Jouer des mouvements enregistr?'}
...
begin
  ...
  MvtAuto            := True;  // On enchaine une s?ie (l'utilisateur ne peut pas faire de modif manuelle)
  iMax               := Length(TheMvts);
  pbFun.Value        := 1;
  pbFun.Max          := iMax;
  pbFun.Visible      := True;
  ...
  MvtAuto            := False; // La s?ie est termin?, l'utilisateur peut ?nouveau jouer
//  pbFun.Visible      := False;
end;
...
/// *************************************************
/// D?lacement de la cam?a autour du rubik's cube
/// *************************************************
